...
# =================== DATABASE CONNECTION ===================
# Using Pooler connection for IPv4 compatibility
...
if selected == "Home":
    st.markdown(
        "<div class='card'><h2>Welcome to Abia State Education Revolution</h2>"
        "<p>Real-time monitoring of education and health across all 17 LGAs.</p></div>",
        unsafe_allow_html=True
    )
    ...
    try:
        # ===== Fixed query with NUMERIC for ROUND =====
        df = pd.read_sql("""
            SELECT 
                l.lga_name,
        ...
        st.error(f"Error fetching live dashboard data: {e}")
        st.stop()

elif selected == "Register/Login":
    st.markdown(
        "<div class='card'><h2>Register / Login</h2>"
        "<p>Registration is by invitation only.<br>Contact: bookyade.abia@gmail.com</p></div>",
        unsafe_allow_html=True
    )

elif selected == "Request Data":
    st.markdown(
        "<div class='card'><h2>Request Full Dataset</h2>"
        "<p>Fill the form below and the complete Excel file will be emailed to you within 24 hours.</p></div>",
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        "<div class='card'><h2>Submit School Data</h2>"
        "<p>Only pre-approved schools can submit. Thank you for helping Abia grow!</p></div>",
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        ...
        "<p>Welcome, BookyAde — Creator & System Owner<br>All systems operational ✅</p></div>",
        unsafe_allow_html=True
    )
